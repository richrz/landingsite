...
interface SolutionsFlyoutProps {
  items?: FlyoutItem[];
}

const defaultItems: FlyoutItem[] = [
  { name: 'Financial Services', path: '/finance' },
  { name: 'Healthcare', path: '/healthcare' },
  { name: 'Government & Defense', path: '/government' },
  { name: 'Pharmaceuticals', path: '/pharma' },
  { name: 'Energy & Utilities', path: '/energy' },
];

const CARROT_WIDTH = 16; // 8px left + 8px right

const SolutionsFlyout: React.FC<SolutionsFlyoutProps> = ({ items = defaultItems }) => {
  const [activeIdx, setActiveIdx] = useState<number | null>(null); // No preselection
          ...
          {carrotTargetIdx !== null && (
            ...
            <motion.div
              ...
              animate={{
                ...
                filter: shouldFlash 
                  ? 'drop-shadow(0 0 8px rgba(228, 228, 231, 0.8))' // Slightly more visible for selection
                  : 'drop-shadow(0 0 4px rgba(139, 92, 246, 0.4))'
              }}
              transition={{ 
                duration: shouldFlash ? 0.2 : 0.1, // Longer flash for selection confirmation
                ease: "easeInOut"
              }}
              style={{ 
          ...
          {items.map((item, idx) => (
            ...
            >
              <button
                ...
                type="button"
                style={{ 
                  border: '1px solid transparent', // Always maintain border to prevent layout shift
                  borderColor: selectedStroke === idx 
                    ? 'rgba(255, 255, 255, 0.8)' // White stroke when selected
                  ...
                  color: (hoveredIdx === idx || activeIdx === idx)
                    ? 'white'
                    : '#b3b3b3',
                  minHeight: '44px', // Fixed height to prevent wobble
                  width: '180px', // Fixed width to prevent wobble
                  minWidth: '180px',
                  boxShadow: selectedStroke === idx 
                    ? '0 0 0 1px rgba(255, 255, 255, 0.6)' // Additional white glow
                    : 'none'
                }}
              >
                {item.name}
